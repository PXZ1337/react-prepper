...
const StockProvider = (props: StockProviderProps) => {
    ...
    const getStocksByParentCategoryId = (parentCategoryId: number): IStockDTO[] =>
        stockState.stocks.filter((stock: IStockDTO) => stock.parentCategoryId === parentCategoryId);
    const getStockById = (stockId: string): IStockDTO => stockState.stocks.filter((stock: IStockDTO) => stock.id === stockId)[0];

    const getStocksBySubCategoryId = (categoryId: string): IStockDTO[] =>
        stockState.stocks.filter((stock: IStockDTO) => stock.categoryId === categoryId);
    ...
    return <StockContext.Provider value={contextProps}>{props.children}</StockContext.Provider>;
};
